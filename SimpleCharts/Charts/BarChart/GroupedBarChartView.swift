...
public class GroupedBarChartView: BaseBarChartView {
    ...
    public var entries: [GroupedEntryModel] = [] {
        didSet {
            ...
            groupedBars = generateBars(entries: entries)
    ...
    fileprivate(set) var groupedBars: [GroupBar] = [] {
        didSet {
            for (index, gb) in groupedBars.enumerated() {
                container.addSublayer(gb.bar.container)
    ...
    override func computeContentSize() -> CGSize {
        ...
        width = max(width, bounds.width - leftSpacing)
        return CGSize(width: width, height: bounds.size.height)
    }

    override func getXPostionForXAxis(for index: Int) -> CGFloat {
        guard let groupId = getGroup(for: index), let firstIndex = groupedBars.first(where: { $0.groupId == groupId })?.bar.data.id else {
    ...
    override func widthForXLabel(for label: String, at index: Int) -> CGFloat {
        guard let groupId = getGroup(for: index) else {
            return super.widthForXLabel(for: label, at: index)
        }

        return CGFloat(groupedBars.filter { $0.groupId == groupId }.count) * (barWidth + spacing)
    }
    ...
    private func getGroup(for index: Int) -> Int? {
        return groupedBars.first(where: { $0.bar.data.id == index })?.groupId
    }
}

extension GroupedBarChartView {
    public func setGroupBarChartOptions(_ chartOptions: [BaseChartViewOption], groupBarOptions: [GroupBarChartViewOption]) {
        super.setChartOptions(chartOptions)

        for option in groupBarOptions {
            switch option {
                case let .barchartOptions(barOptions):
                    // TODO: updating chartOptions twice
                    setBarChartOptions(chartOptions, barOptions: barOptions)
                case let .groupSpacing(value):
                    groupSpacing = value
            }
        }
    }
