...
open class BarContainer: CALayer {
    ...
    public var selected: Bool = false {
        didSet {
            ...
            barLayer?.animateSelected(selected: selected)
    ...
    init(width: CGFloat, color: UIColor, animationDuration: Double, animationDelay: Double, cornerRounding: CGFloat, containerColor: UIColor?) {
        super.init()
        barLayer = BarLayer(width: width, color: color, animationDuration: animationDuration, animationDelay: animationDelay, cornerRounding: cornerRounding)
        barLayer?.barLayerDelegate = self
        self.containerColor = containerColor
        self.cornerRounding = cornerRounding
        backgroundColor = containerColor?.cgColor
    }
    ...
    func setBarLayerHeight(to height: CGFloat, direction: AnimationDirection = .bottom) {
        let bounds = CGRect(x: 0, y: direction == .bottom ? height : 0, width: width, height: self.bounds.size.height - height)
        barLayer?.anchorPoint = direction == .bottom ? CGPoint(x: 1, y: 1) : .zero
...
extension BarContainer: BarLayerDelegate {
    public func animationDidStart() {
        guard let entry = barEntry else { return }
        containerDelegate?.animationDidStart(bar: Bar(container: self, data: entry))
    }

    public func animationDidStop() {
        guard let entry = barEntry else { return }
        containerDelegate?.animationDidStop(bar: Bar(container: self, data: entry))
    }
}
