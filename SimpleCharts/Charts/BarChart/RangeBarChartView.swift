...
public class RangeBarChartView: BaseBarChartView {
    ...
    public var entries: [RangeBarEntryModel] = [] {
        didSet {
            ...
            rangeBars = generateBars(entries: entries)
        }
    }
    ...
    override public var numberOfEntries: Int {
    ...
    fileprivate(set) var rangeBars: [RangeBar] = [] {
        didSet {
            for (index, gb) in rangeBars.enumerated() {
    ...
    override func getXLabel(for index: Int) -> String? {
        guard let groupId = getGroup(for: index), let firstIndex = rangeBars.first(where: { $0.groupId == groupId })?.bar.data else {
    ...
    override func getXPostionForXAxis(for index: Int) -> CGFloat {
        guard let groupId = getGroup(for: index), let firstIndex = rangeBars.first(where: { $0.groupId == groupId })?.bar.data.id else {
            // TODO: What to if group is not found?
            return -100
        }
    ...
    private func getGroup(for index: Int) -> Int? {
        return rangeBars.first(where: { $0.bar.data.id == index })?.groupId
    }
}
