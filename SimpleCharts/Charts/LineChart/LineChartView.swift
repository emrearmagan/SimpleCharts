...
extension LineChartView {
    ...
    private func columnYPoint(_ graphPoint: Int) -> CGFloat {
        ...
        guard let maxValue = graphPoints.map({ $0.value }).max() else { return graphHeight + topBorder }
        // guard let minValue = dataPoints.min() else {return 0}
        guard let lastValue = graphPoints.map({ $0.value }).min() else { return graphHeight + topBorder }

        if graphPoint == lastValue {
            return graphHeight * 0.5 + topBorder
        }
